...
0. Go to the `shared/models/` folder and download the checkpoint for fine-tuning. This
   can be done with `wget https://huggingface.co/lab-cosmo/pet-mad/resolve/v1.0.2/models/pet-mad-v1.0.2.ckpt`.
...
1. Open the `eval.sh` file and setup the export of a PET-MAD checkpoint to the
   ...
   ```bash
   mtt export ../shared/models/pet-mad-v1.0.2.ckpt -o pet-mad-v1.0.2.pt
   ```
...
2. Write the `mtt eval` commands to evaluate the exported model on Li3PS4 and MAD datasets
   ...
   ```bash
   mtt eval pet-mad-v1.0.2.pt eval-options-Li3PS4.yaml -o Li3PS4-sample-test-predictions.xyz -b 8
   mtt eval pet-mad-v1.0.2.pt eval-options-MAD.yaml -o MAD-sample-test-predictions.xyz -b 8
   ```
